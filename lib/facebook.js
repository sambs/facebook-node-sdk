...
Facebook.prototype.sessionPrefix = 'fb_';

Facebook.prototype.setPersistentData = function(key, value) {
  if (this.hasSession) {
    this.request.session[this.sessionPrefix+key] = value;
  }
};

Facebook.prototype.getPersistentData = function(key, defaultValue) {
  if (this.hasSession) {
    return this.request.session[this.sessionPrefix+key] || defaultValue;
  }
  return defaultValue;
};
...
Facebook.prototype.clearPersistentData = function(key) {
  if (this.hasSession) {
    delete this.request.session[this.sessionPrefix+key];
  }
};
  ...
  if (this.hasSession) {
    for (var name in this.sessionNameMap) {
      if (this.sessionNameMap.hasOwnProperty(name)) {
        this.clearPersistentData(this.sessionNameMap[this.sessionPrefix+name]);
      }
    }
  }
};
