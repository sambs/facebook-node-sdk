...
function BaseFacebook(config) {
  ...
  this.accessToken = this.getPersistentData('access_token', null);
  this.code = this.getPersistentData('code', null);
}
  ...
  if (signedRequest) {
    ...
    if (signedRequest.hasOwnProperty('oauth_token')) {
      ...
      if (signedRequest.hasOwnProperty('code')) {
        ...
        var self = this;
        this.getAccessTokenFromCode(code, '', cb.returnToCallback(callback, false, handleAccessTokenFromCode));
      }
      else {
